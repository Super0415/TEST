...
//	  INT08U usBegin = 0;
    ...
    while(1)
    {
        ...
        (void)OSTaskSuspend(OS_PRIO_SELF);
			  if(!usDBfRS485SysMode && (RSRXData == USART1BUFFINFO)) //主机模式 ，且已经接收信息
				{
					ucTmp = SFProcessHostRxModMsg(&sUSART1ModbusMsg);
					usDBmRS485Msg0Norma0 = 1;
			  ...
			  else if(usDBfRS485SysMode)  //从机接收返回数据
			  {
			  ...
			  else if(usFW100msTimer-NowTime > 15)
				{
				...
				{   
					  ...
					  State = Data16uleng | SF_RS485TX_Color1;
					

					  SFUSARTRecordTxMesure(usDBcRS485Sent0DataA0,usDBpRS485Sent0Leng,State);
					  usDBhRS485RXToLeng ++;
				}
...
//    (void)OSIntTaskResume(FW_TASK_PRIO_XIO);		
	...
	//	if((RS485OverTCount>100))   //485超时检测
		{
