...
#define USART1_STEP_INIT         (INT08U)0  //初始化通讯数据
#define USART1_STEP_IDLE         (INT08U)1  //通讯待机中
#define USART1_STEP_RX_ING       (INT08U)2  //正在保存接收数据
#define USART1_STEP_RX_WAIT      (INT08U)3  //等待处理接收数据
#define USART1_STEP_TX_ING       (INT08U)4  //正在写入发送数据
#define USART1_STEP_TX_WAIT      (INT08U)5  //等待处理发送数据
INT08U USART1CtrlStep;  //USART1通讯进度
...
void USART1_Init(void)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA|RCC_APB2Periph_AFIO|RCC_APB2Periph_USART1,ENABLE);	//串口功能 管脚复用 串口时钟 
...
//暂时只提供主机模式-用于主动发送数据，  从机模式-可用于后期制作上位机
				if(!usDBfRS485SysMode)
				{
						
						SFProcessHostRxModMsg(&sUSART1ModbusMsg);
				}
				else
				{
...
void USART1SentData(INT16U Length,INT16U * DataAddrese,INT16U CRCCMD)
{
	  ...
	  for(i = 0;i < Length;i++)
	  {
			  ...
			  if(USART_GetITStatus(USART1,USART_FLAG_TC)!=Bit_SET)  //查询 USART_FLAG_TC 并清零
			  {
