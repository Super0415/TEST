...
{   INT08U ucComd, ucErr;
    while (1)
    {
        if(usDBfSysBackup == 1)
				...
				else if(usDBfSysBackup == 2)  //清除485缓存报文数据
				{
						usDBfSysBackup = 0;
						(void)SFFill16u(aDBUSARTHstry,0,DB_USARTHSTRY_SIZE);
				}
				else if(usDBfSysBackup == 3)  //清除CAN缓存报文数据
				{
						usDBfSysBackup = 0;
...
void DBDataInit(void)
{
    usDBfRS485EnSentData0 = 1;  //第一条发送使能
	  usDBpRS485Sent0delay = 1000; //第一条发送时间间隔
    usDBpRS485Sent0Leng = 8;    //第一条发送长度
	  *usDBcRS485Sent0DataA0     = 0x01;  //位置0    01 03 00 00 00 01 84 0A
    *(usDBcRS485Sent0DataA0+1) = 0x03;  //位置1
	  *(usDBcRS485Sent0DataA0+2) = 0x00;  //位置2
	  *(usDBcRS485Sent0DataA0+3) = 0x00;  //位置3
	  *(usDBcRS485Sent0DataA0+4) = 0x00;  //位置4
	  *(usDBcRS485Sent0DataA0+5) = 0x01;  //位置5
	  *(usDBcRS485Sent0DataA0+6) = 0x84;  //位置6
	  *(usDBcRS485Sent0DataA0+7) = 0x0A;  //位置7
	
	usDBfCANEnable = 0;  //CAN使能
}
