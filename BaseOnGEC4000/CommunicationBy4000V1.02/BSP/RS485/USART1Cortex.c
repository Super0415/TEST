...
{
	  INT08U ucTmp;
    INT08U Data16uleng,usBegin,usPar;
	  INT16U Correct = 0;	
    ...
    while(1)
    {
        ...
        (void)OSTaskSuspend(OS_PRIO_SELF);
			  ...
			  else if(usDBfRS485SysMode)  //从机接收返回数据
			  {
				...
				if(TESTZC[0]/*usDBfRS485EnSentData0*/ && RSOnTimeSign0) //第0条  
				{   TESTZC[0] = 0;
						RSOnTimeSign0 = 0;
					  ...
					  usBegin = usDBhRS485RXToLeng;  //记录报文位置
            usPar = Parity(sUSART1ModbusMsg.ucLeng);
	          Data16uleng = (sUSART1ModbusMsg.ucLeng)/2 + usPar+1;  //多出一位用于记录报文信息					
					  Correct = Data16uleng | SF_RS485TX_Color1;
					
					TESTZC[1] = usBegin;
					TESTZC[2] = usPar;
					TESTZC[3] = Data16uleng;
					TESTZC[4] = Correct;
					  SFUSARTRecordMesure(usBegin,&sUSART1ModbusMsg,Data16uleng,Correct);
					  usDBhRS485RXToLeng += Data16uleng;
				}
...
void Tim2IntHook(void)
{
  ...
  if(1/*usDBfRS485EnSentData0==1*/ && (Time2Count-RSOnTime0)>100/*usDBpRS485Sent0delay*/) //第0条发送报文
	{
		  RSOnTime0 = Time2Count;
		  RSOnTimeSign0 = 1;
	    (void)OSIntTaskResume(FW_TASK_PRIO_SCI1); //启动SCI1
	}
  if(usDBfRS485EnSentData1==1 && (Time2Count-RSOnTime1)>usDBpRS485Sent1delay) //第1条发送报文
	{
		  RSOnTime1 = Time2Count;
		  RSOnTimeSign1 = 1;
	    (void)OSIntTaskResume(FW_TASK_PRIO_SCI1); //启动SCI1
	}
  if(usDBfRS485EnSentData2==1 && (Time2Count-RSOnTime2)>usDBpRS485Sent2delay) //第2条发送报文
	{
		  RSOnTime2 = Time2Count;
		  RSOnTimeSign2 = 1;
	    (void)OSIntTaskResume(FW_TASK_PRIO_SCI1); //启动SCI1
	}
  if(usDBfRS485EnSentData3==1 && (Time2Count-RSOnTime3)>usDBpRS485Sent3delay) //第3条发送报文
	{
		  RSOnTime3 = Time2Count;
		  RSOnTimeSign3 = 1;
	    (void)OSIntTaskResume(FW_TASK_PRIO_SCI1); //启动SCI1
	}
  if(usDBfRS485EnSentData4==1 && (Time2Count-RSOnTime4)>usDBpRS485Sent4delay) //第4条发送报文
	{
		  RSOnTime4 = Time2Count;
		  RSOnTimeSign4 = 1;
	    (void)OSIntTaskResume(FW_TASK_PRIO_SCI1); //启动SCI1
	}
  if(usDBfRS485EnSentData5==1 && (Time2Count-RSOnTime5)>usDBpRS485Sent5delay) //第5条发送报文
	{
