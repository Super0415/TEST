...
INT08U SFProcessHostRxModMsg (MODBUS_MSG *pMsg)
{
    ...
    usPar = Parity(pMsg->ucLeng);
	  Data16uleng = (pMsg->ucLeng)/2 + usPar+1;  //多出一位用于记录报文信息
		usBegin = usDBhRS485RXToLeng;  //记录报文位置
		//Correct 的低8位 记录报文长度， 高8位 中前4位记录报文类型区分是发送还是接收，后四位记录报文颜色
		Correct = Data16uleng | SF_RS485RX_Color0;
		SFUSARTRecordMesure(usBegin,pMsg,Data16uleng,Correct); //记录接收到的报文
...
//说明：
//备注：
void SFUSARTRecordMesure(INT08U pDat, MODBUS_MSG *pMsg,INT16U uLeng, INT16U usSign)
{  
	  INT08U i = 0;
	  for(i = 0;i < uLeng-1;i++)
	  {
        aDBMeasu[DB_MEASU_NOR_SIZE+1+pDat+i] = SF_UC_ToHIGH(pMsg->aData[i*2]) | SF_UC_ToLow(pMsg->aData[i*2+1]);
		}
		aDBMeasu[pDat+uLeng] = usSign;
}
