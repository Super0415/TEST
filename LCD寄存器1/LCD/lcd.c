...
void TFT_ClearScreen(uint16_t color)
{
	...
	LCD_open_windows(0, 0, TFT_XMAX, TFT_YMAX);	 //作用区域
  	for(i=0; i<TFT_XMAX+1; i++)
	{
		for (j=0; j<TFT_YMAX+1; j++)
		{
			LCD_ColorPoint(j,i,color);
		}
	}
}
void TFT_ClearSpecailScreen(uint16_t X,uint16_t Y,uint16_t Len,uint16_t Wid,uint16_t color)
{
 	uint16_t i, j ;

	LCD_open_windows(X, Y, Len, Wid);	 //作用区域
  	for(i=Y; i<Y+Wid+1; i++)
	{
		for (j=X; j<X+Len+1; j++)
		{
