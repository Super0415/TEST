...
{   INT16U usTmp;
    ...
    { TESTZC1[0] = usTmp;
      ...
      TESTZC1[1] = usTmp;
        ...
        {   SF_ERR1_SET_BIT(SF_ERR1_OIL_PRES_VT);                     //置油压断线报警
            usDBmOilPressure = usDBpSubstOilPress;
            TESTZC1[2]++;
        }   else
        {   SF_ERR1_CLR_BIT(SF_ERR1_OIL_PRES_VT);                    //取消油压断线报警
...
{   INT16U usBin        =  usInput;
    ...
    if(usDBmKeyEngineStop==1)TESTZCKey[0]++;
    ...
    usDBmKeyErrorReset  = (usBin >> SF_KEY_BIT_ERROR_RSET) & 1;
...
{   static INT16U usStart  = 0;
    ...
    usTmp = SFGet1DMap10(usDBmSpeed, &aDBpOilPressStopSpeed[0],
                                     &aDBpOilPressStopPress[0]);   //通过速度与油压的关系，得出当前速度下的油压停机临界值
    if ((usStart == 0) || (usDBmOilPressure >= usTmp))             //油压超过临界值，记录当前时间值
    {   usTimer2 = usTimNow;
    }
    else if ((INT16U)(usTimNow-usTimer2) > usDBpOilPressStopDly)   //时间差达到设定值，置油压停机报警
    {   SF_ERR2_SET_BIT(SF_ERR2_OIL_PRESS_STOP);
    }
    /* ************************* Pressure Warn ************************ */
    usTmp = SFGet1DMap10(usDBmSpeed, &aDBpOilPressWarnSpeed[0],
                                     &aDBpOilPressWarnPress[0]);   //通过速度与油压的关系，得出当前速度下的油压报警临界值
    if (usStart == 0)
