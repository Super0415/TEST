...
{   
  ...
  if(KEY_MODE_STOP == 0){TESTZC[0]++;  return  1<<SF_KEY_BIT_ENG_STOP;}
  if(KEY_MODE_MANUAL == 0){TESTZC[1]++;  return  1<<SF_KEY_BIT_MODE_MANUAL;}
...
void XIOInitDigInput (void)
{
 ...
 //PE6-停机按钮
  ...
  //设置PE6为上拉输入
    ...
    GPIOE->ODR |= (0x07 << 6);  
  */
   XIOConfigGPIOKEY(GPIOE, XIO_PT_BT6, XIO_PT_GPI_PULL, XIO_PT_SET_1); 
   XIOConfigGPIOKEY(GPIOE, XIO_PT_BT5, XIO_PT_GPI_PULL, XIO_PT_SET_1);  
