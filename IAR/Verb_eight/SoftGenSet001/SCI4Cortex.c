...
{   INT16U usTmp;
    ...
    {   (void)OSTaskSuspend(OS_PRIO_SELF);
        ...
        else if (usTmp <= SCI4_TX_BUF_SIZE)
        {   (void)SCI4SetupRxDma((INT08U*)&aSCI4RxBuf[0], SCI4_RX_BUF_SIZE*2);
            (void)SCI4SetupTxDma((INT08U*)&aSCI4TxBuf[0], usTmp*2);
        }
        else
        {   SCI4_SCI_TX_DIS();
            SCI4_SCI_RX_DIS();
            (void)OSTimeDly((OS_TICKS_PER_SEC/SCI4_TASK_FRQ)/2);
            (void)SCI4Reset();
        }
    }
}
...
{   OSTimeDly(OS_TICKS_PER_SEC * 3);
    ...
    if (SCI4ReadRxBuf(&aSCI4RxBuf[0], SCI4_CODE_SIZE_PARAM+4) == SCI4_FALSE)
    ...
    SCI4SetupTxDma((INT08U*)&aSCI4TxBuf[0], SCI4_TX_BUF_SIZE*2);
    OSTimeDly(OS_TICKS_PER_SEC / SCI4_TASK_FRQ);
    if (SCI4ReadRxBuf(&aSCI4RxBuf[0], SCI4_CODE_SIZE_FUNCT+4) == SCI4_FALSE)
    {   SCI4_STS_SET_BIT(SCI4_STS_ERR_RX);
    }
    ...
    SCI4SetupTxDma((INT08U*)&aSCI4TxBuf[0], SCI4_TX_BUF_SIZE*2);
    OSTimeDly(OS_TICKS_PER_SEC / SCI4_TASK_FRQ);
    if (SCI4ReadRxBuf(&aSCI4RxBuf[0], SCI4_CODE_SIZE_CURVE+4) == SCI4_FALSE)
    {   SCI4_STS_SET_BIT(SCI4_STS_ERR_RX);
    }
    while (1)
    {   SCI4SetTxBuf(&aSCI4TxBuf[0], SCI4_CODE_BEGIN_MEASU, 
        ...
        SCI4SetupTxDma((INT08U*)&aSCI4TxBuf[0], SCI4_TX_BUF_SIZE*2);
        OSTimeDly(OS_TICKS_PER_SEC / SCI4_TASK_FRQ);
        if (SCI4ReadRxBuf(&aSCI4RxBuf[0], SCI4_CODE_SIZE_MEASU+4) == SCI4_FALSE)
        {   SCI4_STS_SET_BIT(SCI4_STS_ERR_RX);
        }
    }
}
...
/* ******************************************************************** */
#if (SCI4_MASTER_FUNCTION_ON == 1)
INT08U SCI4ReadRxBuf (INT16U *pRxBuf, INT16U usRxSiz)
{   INT16U usBegin, usSize;
    if ((pRxBuf[0] != SCI4_CODE_RESPOND) || (usRxSiz < 4))
    {   pRxBuf[0]  = 0;
        return SCI4_FALSE;
    }
    usBegin = SFGetSum16u(&pRxBuf[0], usRxSiz-1);
    if (pRxBuf[usRxSiz-1] != usBegin)
    {   pRxBuf[0] = 0;
        return SCI4_FALSE;
    }
    pRxBuf[0] = 0;
...
{   INT16U usBegin, usSize;
    ...
    {   if (usBegin >= (DB_PARAM_SIZE+DB_FUNCT_SIZE+DB_CURVE_SIZE))
        {   usBegin -= (DB_PARAM_SIZE+DB_FUNCT_SIZE+DB_CURVE_SIZE);
            if (DBCheckMeasu16p(&pRxBuf[5], usBegin, usSize) == DB_DIFFERENT)
            {   DBSetMeasu16p(&pRxBuf[5], usBegin, usSize);
            }
        }
        else if (usBegin >= (DB_PARAM_SIZE+DB_FUNCT_SIZE))
        {   usBegin -= (DB_PARAM_SIZE+DB_FUNCT_SIZE);
            if (DBCheckCurve16p(&pRxBuf[5], usBegin, usSize) == DB_DIFFERENT)
        ...
        {   usBegin -= DB_PARAM_SIZE;
            if (DBCheckFunct16p(&pRxBuf[5], usBegin, usSize) == DB_DIFFERENT)
            {   DBSetFunct16p(&pRxBuf[5], usBegin, usSize);
                DBTskPutCmd(DB_CMD_SAVE_FUNCT);
            }
        }
        else if (DBCheckParam16p(&pRxBuf[5], usBegin, usSize) == DB_DIFFERENT)
    ...
    {   usBegin -= (DB_PARAM_SIZE+DB_FUNCT_SIZE+DB_CURVE_SIZE);
        ...
        usBegin  =  DBGetMeasuAndSum16p(&pTxBuf[3], usBegin, usSize);
    }
    else if (usBegin >= (DB_PARAM_SIZE+DB_FUNCT_SIZE))
    {   usBegin -= (DB_PARAM_SIZE+DB_FUNCT_SIZE);
        ...
        usBegin  =  DBGetCurveAndSum16p(&pTxBuf[3], usBegin, usSize);
    }
    else if (usBegin >= DB_PARAM_SIZE)
    {   usBegin -=  DB_PARAM_SIZE;
        ...
        usBegin  =  DBGetFunctAndSum16p(&pTxBuf[3], usBegin, usSize);
    }
    else
    {   if (((INT32U)usBegin+(INT32U)usSize) > (INT32U)DB_PARAM_SIZE)
        {   return 0xFFFF;
        }
        usBegin  =  DBGetParamAndSum16p(&pTxBuf[3], usBegin, usSize);
    }
    pTxBuf[usSize+3] = usBegin + pTxBuf[0] + pTxBuf[1] + pTxBuf[2];
    return usSize+4;
}
#endif
...
{   RCC->APB1ENR  |=  RCC_APB1ENR_UART4EN;
    ...
    XIOConfigGPIO(SCI4_RX_PT, SCI4_RX_BT, XIO_PT_GPI_PULL,   XIO_PT_SET_1);
