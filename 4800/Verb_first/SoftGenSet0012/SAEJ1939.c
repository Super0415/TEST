...
/*                           SAECheckTime ()                            */
/* ******************************************************************** */
INT08U SAECheckTime (INT16U usOld, INT16U usPer)
{   INT16U usTemp;
    usTemp  = usSAETimer;
    usTemp -= usOld;
    if (usTemp >= usPer)
...
{   INT08U  i, ucFlag;
    ...
    {   pTcb = &sSAERxPgnTbl[i];
        ...
        {   if (SAECheckTime(pTcb->usTime,pTcb->usPeriod) != SAE_FALSE)
...
{   INT08U  i;
    ...
    {   pTcb = &sSAETxPgnTbl[i];
        ...
        else if (SAECheckTime(pTcb->usTime,pTcb->usPeriod) != SAE_FALSE)
        {   pTcb->usTime = usSAETimer;
        }
        else
        {   continue;
...
void SAEUpdateRxEEC1 (CAN_MSG *pMsg)
{
    ...
    INT16U usTemp;
    usTemp = SAE_US_CREAT (pMsg->DAT08.ucDat3, pMsg->DAT08.ucDat4);
    if (usTemp <= 64255)
...
void SAEUpdateRxVEHSPD (CAN_MSG *pMsg)
{
    ...
    INT16U usTemp;
    usTemp = SAE_US_CREAT (pMsg->DAT08.ucDat1, pMsg->DAT08.ucDat2);
    if (usTemp <= 64255)
...
void SAEUpdateRxENGTEMP (CAN_MSG *pMsg)
{
    ...
    usTemp = SAE_US_CREAT (pMsg->DAT08.ucDat2, pMsg->DAT08.ucDat3);
    if (usTemp <= 64255)
...
void SAEUpdateRxLFE (CAN_MSG *pMsg)
{
    ...
    INT16U usTemp;
    usTemp = SAE_US_CREAT (pMsg->DAT08.ucDat0, pMsg->DAT08.ucDat1);
    if (usTemp <= 64255)
...
void SAEUpdateRxINCON (CAN_MSG *pMsg)
{
   ...
   #if (SAE_1000_TWO_FUEL_ON == 1)
    INT16U usTemp;
    usTemp = SAE_US_CREAT (pMsg->DAT08.ucDat4, pMsg->DAT08.ucDat5);
...
void SAEUpdateRxVEHPOW (CAN_MSG *pMsg)
{
    ...
    INT16U usTemp;
    usTemp = SAE_US_CREAT (pMsg->DAT08.ucDat6, pMsg->DAT08.ucDat7);
    if (usTemp <= 64255)
...
{   INT16U usTemp;
    usDBmCanSetpoint1 = SAE_US_CREAT (pMsg->DAT08.ucDat1, pMsg->DAT08.ucDat2);
    usDBmCanSetpoint2 = SAE_US_CREAT (pMsg->DAT08.ucDat6, pMsg->DAT08.ucDat7);
    usDBmCanSetpoint3 = SAE_US_CREAT (pMsg->DAT08.ucDat0, pMsg->DAT08.ucDat3);
    usTemp            = SAE_US_CREAT (pMsg->DAT08.ucDat4, pMsg->DAT08.ucDat5);
    usTemp           &= 0x7FFF;
