...
/* ******************************************************************** */
void LED_SetState(GPIO_TypeDef *pPt,INT08U ucBit,u8 usSet)
{    
    if (usSet    ==  0)
...
void XIOUpdateSwitchesOutput (INT16U usOutput)  //TESTSIGN
{   
    LED_SetState(GPIOB, XIO_PT_BT8,(usOutput>>0)&0x1);      //输出口1 PB8   OUT1
    LED_SetState(GPIOB, XIO_PT_BT3,(usOutput>>1)&0x1);      //输出口2 PB3   OUT2    
    LED_SetState(GPIOB, XIO_PT_BT6,(usOutput>>2)&0x1);      //输出口3 PB6   OUT3
    LED_SetState(GPIOB, XIO_PT_BT7,(usOutput>>3)&0x1);      //输出口4 PB7   OUT4    
    LED_SetState(GPIOC, XIO_PT_BT5,(usOutput>>4)&0x1);      //输出口5 PC5   OUT5    
    LED_SetState(GPIOC, XIO_PT_BT4,(usOutput>>5)&0x1);      //输出口6 PC4   OUT6   
    
//    LED_SetState(GPIOB, XIO_PT_BT8,(usOutput>>6)&0x1);      //输出口7 PB8
//    LED_SetState(GPIOB, XIO_PT_BT8,(usOutput>>7)&0x1);      //输出口8 PB8
//    LED_SetState(GPIOB, XIO_PT_BT8,(usOutput>>8)&0x1);      //输出口9 PB8
//    LED_SetState(GPIOB, XIO_PT_BT8,(usOutput>>9)&0x1);      //输出口10 PB8
//    LED_SetState(GPIOB, XIO_PT_BT8,(usOutput>>10)&0x1);      //输出口11 PB8
//    LED_SetState(GPIOB, XIO_PT_BT8,(usOutput>>11)&0x1);      //输出口12 PB8
//    LED_SetState(GPIOB, XIO_PT_BT8,(usOutput>>12)&0x1);      //输出口13 PB8
//    LED_SetState(GPIOB, XIO_PT_BT8,(usOutput>>13)&0x1);      //输出口14 PB8    
    
    LED_SetState(GPIOA, XIO_PT_BT15,(usOutput>>14)&0x1);     //输出口15 PA15   CRANK  
    LED_SetState(GPIOB, XIO_PT_BT4,(usOutput>>15)&0x1);      //输出口16 PB4   D+    
...
/* ******************************************************************** */
int TESTZCXIO[10];
void XIOInitDigOutput (void)     //TESTSIGN
{
  
    RCC->APB2ENR |= RCC_APB2ENR_AFIOEN;
    TESTZCXIO[0] = AFIO->MAPR;
    AFIO->MAPR   |= AFIO_MAPR_SWJ_CFG_JTAGDISABLE;
    TESTZCXIO[1] = AFIO->MAPR;
    TESTZCXIO[2] = AFIO_MAPR_SWJ_CFG_JTAGDISABLE;
    AFIO->MAPR2  |= 0; 
...
{   XIOConfigGPIO(GPIOA, XIO_PT_BT0, XIO_PT_GPI_ANALOG, XIO_PT_SET_0);//端口初始化-GEC4800悬空
 ...
 //   XIOConfigGPIO(GPIOC, XIO_PT_BT4, XIO_PT_GPI_ANALOG, XIO_PT_SET_0);  //有端口功能重合
 //   XIOConfigGPIO(GPIOC, XIO_PT_BT5, XIO_PT_GPI_ANALOG, XIO_PT_SET_0);  //有端口功能重合
