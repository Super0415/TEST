...
{   CAN1_TC_INT_DIS();
    CAN1_RX_INT_EN();
    FWIrqEnable(USB_HP_CAN1_TX_IRQn , 0);  //@@
    FWIrqEnable(USB_LP_CAN1_RX0_IRQn, 0);  //@@
...
/* ******************************************************************** */
#if (CAN1_FUNCTION_ON == 1)
void CAN1PutRxMsgToBuf (CAN_MSG *msg)
{   CAN1_RING *pbuf;
    CAN_MSG *pmsg, *ptmp;
    pbuf = &sCAN1RingBuf;
...
/* ******************************************************************** */
#if (CAN1_FUNCTION_ON == 1)
INT08U CAN1GetTxMsgFmBuf (CAN_MSG *msg)
{   CAN1_RING *pbuf;
    CAN_MSG *pmsg, *ptmp;
    pbuf = &sCAN1RingBuf;
...
/* ******************************************************************** */
#if (CAN1_FUNCTION_ON == 1)
INT08U CAN1GetMsgFmRxBuf (INT16U to, CAN_MSG *msg)
{   OS_CPU_SR cpu_sr = 0;
    INT08U oserr;
    CAN1_RING *pbuf;
    CAN_MSG *pmsg, *ptmp;
...
/* ******************************************************************** */
#if (CAN1_FUNCTION_ON == 1)
INT08U CAN1PutMsgToTxBuf (INT16U to, CAN_MSG *msg)
{   OS_CPU_SR cpu_sr = 0;
    INT08U oserr;
    CAN1_RING *pbuf;
    CAN_MSG *pmsg, *ptmp;
...
/* ******************************************************************** */
#if (CAN1_FUNCTION_ON == 1)
void CAN1HardInit (void)
{   RCC->APB2ENR  |=  RCC_APB2ENR_AFIOEN;
    RCC->APB2ENR  |=  RCC_APB2ENR_IOPDEN;
    RCC->APB1ENR  |=  RCC_APB1ENR_CAN1EN;
    RCC->APB1RSTR |=  RCC_APB1RSTR_CAN1RST;
    RCC->APB1RSTR &= ~RCC_APB1RSTR_CAN1RST;
    XIOConfigAFIO(XIO_Remap2_CAN1);
//    XIOConfigGPIO(GPIOD, XIO_PT_BT0, XIO_PT_GPI_PULL,   XIO_PT_SET_1);      //Rx PA11   Tx PA12
//    XIOConfigGPIO(GPIOD, XIO_PT_BT1, XIO_PT_ALT_PP_50M, XIO_PT_SET_0);
    XIOConfigGPIO(GPIOA, XIO_PT_BT11, XIO_PT_GPI_PULL,   XIO_PT_SET_1);      //Rx PA11   Tx PA12
    XIOConfigGPIO(GPIOA, XIO_PT_BT12, XIO_PT_ALT_PP_50M, XIO_PT_SET_0);    
   ...
   #elif (CAN1_BAUD_RATE_KHZ == 500)
    CAN1->BTR  = ((INT32U)0 << 30)          /* 0: Normal operation mode */
              +  ((INT32U)1 << 24)          /* 1: Resynch jump width= 2 */
              +  ((INT32U)5 << 20)          /* 5: Time segment 2 = 6tq  */
              +  ((INT32U)8 << 16)          /* 8: Time segment 1 = 9tq  */
              +  ((INT32U)9 - 1);           /* Prescaler=9:  Tq = 1/8us */
   #elif (CAN1_BAUD_RATE_KHZ == 1000)
    CAN1->BTR  = ((INT32U)0 << 30)          /* 0: Normal operation mode */
    ...
    CAN1->FA1R =  0x00000000;               /* All Filters not active   */
