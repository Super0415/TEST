...
{   INT16U usPhase, usSpeed;
    usSpeed                  = usDBmEngSpeed;                //获取发动机实时转速
    usDBmEngEngineIsStopped  = SFGetEngStsStopped(usSpeed);  //获取发动机停机状态
    usDBmEngEngineIsRunning  = SFGetEngStsRunning(usSpeed);  //获取发动机运行状态
    usDBmEngEngineIsIdling   = SFGetEngStsIdling(usSpeed);   //获取发动机怠速状态
    usDBmEngEngineIsRated    = SFGetEngStsRated(usSpeed);    //获取发动机速率 @@@@ZC？？
    if (usDBmSwitchEmergStop != 0)    //开关量功能项-停机
    {   usDBmSysEmergnCmdToStop = 1;  //系统停机指令
    }
    else if (usDBmEngEngineIsStopped != 0)  //发电机不处于停机状态
    {   usDBmSysEmergnCmdToStop = 0;  //系统不发出停机指令                @@@ZC - 此处功能与预期不符 机组有转速时，接上控制器，因启动停机功能
    }
...
{   if (ERR_ES1_GET_BIT(ERR_ES1_EMERG_MSK) != 0)  //市电电压报警、停机报警、可修改屏蔽值
    {   return 1;
    }
    if (ERR_ES3_GET_BIT(ERR_ES3_EMERG_MSK) != 0)  //发电电压报警、停机报警
    {   return 2;
    }
    if (ERR_ES5_GET_BIT(ERR_ES5_EMERG_MSK) != 0)  //传感器报警、停机报警
    {   return 3;
    }
    if (ERR_ES6_GET_BIT(ERR_ES6_EMERG_MSK) != 0)  //杂项备用报警、杂项备用停机报警
    {   return 4;
    }
    if (ERR_ES2_GET_BIT(ERR_ES2_EMERG_MSK) != 0)  //市电电流报警、停机报警、备用项
    {   return 5;
    }
